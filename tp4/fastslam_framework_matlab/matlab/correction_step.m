...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      particles(i).landmarks(l).mu = T*[z(l).range*sin(z(l).bearing);
                                        z(l).range*cos(z(l).bearing);
                                                     1              ];
      ...
      Q = inv(H)*particles(i).landmarks(l).sigma*inv(H)' +  Q_t;
      ...
      diffZ = [z(j).range - expectedZ(1)  ;
               normalize_angle(z(j).bearing - expectedZ(2))];  %% VERRRRRRR
      ...
      particles(i).weight = particles(i).weight * (abs(2*pi*Q))^(-0.5)*exp(-.5*diffZ'*inv(Q)*diffZ);
